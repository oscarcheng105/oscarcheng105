...
static block_t *find_fit(size_t asize) {
    ...
    for (b = root; b != NULL ; b = b -> body.next ) {
...
static block_t *coalesce(block_t *block) {
    ...
    if (prev_alloc && next_alloc) { /* Case 1 */
        ...
        //////////// Remove next and prev block to coelesce
        block_t* next_block = (block_t*)next_header;
	    remove_block(next_block);
	    remove_block(prev_block);
	    ////////////
...
static void add_block(block_t* block){
	...
	if(root == NULL) //if free list is empty
	{
		block -> body.next = NULL;
		block -> body.prev = NULL;
		root = block;
	}
	else //add to free list through LOFI method
	{
		root -> body.prev = block;
		block -> body.next = root;
		block -> body.prev = NULL;
		root = block;
	}
}
...
static void remove_block(block_t* block){
	...
	if(block -> body.prev == NULL && block -> body.next == NULL) //if block is at beginning and only block of free list
	{
		root = NULL;
		block -> body.next = NULL;
	}
	else if(block -> body.prev == NULL && block -> body.next != NULL) //if block is at beginning of free list
	{
	...
	else if(block -> body.prev != NULL && block -> body.next != NULL) //if block is in middle of free list
	{
	...
	else if(block -> body.prev != NULL && block -> body.next == NULL) //if block is at end of free list
	{
